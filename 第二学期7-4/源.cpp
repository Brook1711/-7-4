...
int main()
{
	...
	vector<int> temp_int;
	vector<BiNode<int>*>v;
	BiTree<int>* arrTree = new BiTree<int>[size];
	for(int i=0;i<size;i++)
	{
		cin >> temp_data;
		temp_int.push_back(temp_data);
	}
	sort(temp_int.begin(),temp_int.end());	//too young too naive!
	reverse(temp_int.begin(), temp_int.end()); //你以为一个排序可以解决问题？！
	for (int i = 0; i < size; i++)
	{
		int tempInt = temp_int[i];
		arrTree[i].SimpleBiTree(tempInt);
		v.push_back(arrTree[i].root);
	}
	BiTree<int> FinalTree;
	FinalTree.CreateHuffman(v);
	cout << FinalTree.GetHuffmanNum(FinalTree.root) - FinalTree.root->data;
